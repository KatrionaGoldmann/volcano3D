...
mytable = data.frame(
  ...
  pvalues = c("pvalues\ 
              ...
              column names of format `paste0(groups[i], '_', groups[j], '_', p_col_suffix)`.\ 
              We recommend \
              using 'limma' or 'DESeq' pipelines to calculate these pvalues for \
              gene expression.\ 
              \n * _optional_ fold change columns: one for each comparison with\
              column names of format `paste0(groups[i], '_', groups[j], '_', fc_col_suffix)`  \
              \n * _optional_ adjusted pvalue columns: one for each comparison\
              with column names of format `paste0(groups[i], '_', groups[j], '_', padj_col_suffix)` \
...
```{r, eval = FALSE}
orca(p, "~/Desktop/volcano_3d_synovium.svg", format = "svg")
```

We can alter the colours using the colours parameter. These colours are assigned
in order
...
We can inspect the grid using `show_grid()`:
