...
groups.
4. the polar coordinates to plot the differentially expressed data. 
...
generated using ANOVA or likelihood ratio tests between all three groups.
  * three _optional_ fold change columns: one for each comparison between groups
...
of interestt at plotting stage. If NULL the rownames will be used. 
...
For example we can descrease the number of radial spokes to 4, while altering 
the z axis ticks:
...
similarly to extend the radial aixs and increasing the number of spokes in 2D
we can apply: 
...
interest. This can be alrtered by amending the label column in the polar object:
