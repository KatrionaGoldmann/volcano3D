...
| <b>comparison</b> | two groups between which a statistical test can be performed. There are three comparisons total (since 3!/2 = 3). For the examples outlined in this vignette we look at comparisons: 'lymphoid-myeloid', 'lymphoid-fibroid' and 'myeloid-fibroid' as these are two groups in the pathotype contrast. |
| <b>p</b> | p value |
| <b>FC</b> | fold change |
...
| <b>dep</b> | '<em>D</em>ifferential <em>E</em>xpression <em>P</em>-values' object, of S4 class, containing the expression data, sample data and pvalues. |
...
To create a dep object you must provide the create_dep function:

* `sampledata`: A data frame containing the sample information. This must 
contain:
  * an `ID` column: Containing the sample IDs. This must be titles 'ID'. 
...
* `pvalues`: A data frame containing:
    * three pvalue columns: one for each comparison between groups (column names 
    of format `paste0(groups[i], "-", groups[j], " ", pColSuffix)`). We 
...
The `sig` column in `syn_polar@polar` allows us to determine relative differences 
in expression between pathotypes. The '+' 
...
By hovering over certain point you can also determine which genes these are for 
future interrogation. 

Similarly we can create a ggplot static image using radial_ggplot:
...
Note that these are ggplot objects so can be easily altered. 
...
Again this produces an interactive plot which can be interrogated. If you have 
...
pvalues were generated using [QuSAGE methodology](https://www.ncbi.nlm.nih.gov/
pubmed/23921631).
